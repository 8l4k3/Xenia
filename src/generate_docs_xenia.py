...
# === CONFIGURATION ===
md_file = Path("fiches/fiche_utilisateur.md")
output_pdf = Path(f"fiches/{md_file.stem}.pdf")
output_docx = Path(f"fiches/{md_file.stem}.docx")
qr_folder = Path("qrcode")
qr_folder.mkdir(exist_ok=True)
qr_file = qr_folder / f"{md_file.stem}_QR.png"

# === 1️⃣ Génération QR code pour la fiche ===
...
# === 3️⃣ Génération PDF avec mise en page simple ===
...
# === 4️⃣ Génération DOCX avec QR code mentionné ===
doc = Document()
